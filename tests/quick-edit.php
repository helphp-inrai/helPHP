...
class Test extends helPHP_Module
{
    ...
    public function process_data(&$post)
    {
        ...
        switch ($post[$this->input_action_identifier]) {
            default:
                $this->display=H::new_document('quick edit test', '', '', false, true);
    ...
    public function testing($post){
        ...
        $form = H::form(['action'=>'http://localhost/tests/quick-edit.php', 'dom_target'=>$this->css.'result', 'style'=>$style]);
            $title = H::DIV([], 'test quick edit feature<br> you must be connected to test it<br><br> double click / tap on text bellow to change your firstname');
        $form->add_child( $title );
            $my_quick_edit = H::quick_edit($form, $USER->id, 'users_data-firstname', ['class'=>'myqe'],$USER->user_data['firstname'], 'string');
            $result = H::DIV(['id'=>$this->css.'result']);
              $my_quick_edit2 = H::quick_edit($form, 10, 'users_data-vat_id', ['class'=>'myqe','placeholder'=>'VAT ID'],$USER->user_data['vat_id'], 'string');
        $form->add_child([$my_quick_edit,$my_quick_edit2,$result]);
